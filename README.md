...
| `compile_rule` |       `1mb-rule`        | 11.421 ms | 1MB 大小的规则编译（解析）                 |
| `rule_match`   |    `worst-1mb-rule`     | 11.502 ms | 1MB 大小的规则匹配（匹配到末尾的最糟情况） |

如上所见，正常或正常稍长的规则都能在纳秒级的速度内完成匹配。即使规则文本数据有 1MB 大小（可能有数万行）也能在 11 毫秒解析完成或匹配结束。

规则的最终的目的和正则表达式有部分重叠，但正则是开销不恒定的东西。在几乎任何系统的设计上都不建议允许用户直接输入的正则表达式，因为攻击者能利用病态正则（专门写出的速度特别慢的正则）轻易的将系统资源耗光，哪怕是 Cloudflare 也曾因此出过事故。并且实际上正则也并不适合对消息进行条件化的复杂匹配，它适合对单个关键字实施更精准的匹配（并且如上原因，它不应该被支持）。

匹配引擎恒定的开销意味着无论用户输入怎样的规则，都不会影响系统的稳定性。随着规则数量的与日俱增也只会让系统的开销呈稳定的线性增长。
...
例如匹配消息文本（`message.text`）或匹配消息来源（`message.from`）。不过消息来源自身没有可匹配的内容，应该访问其具体字段，例如来源是否是机器人（`message.from.is_bot`）或消息来源用户的名字（`mesage.from.fullname`）。

**注意**：字段和 Telegram 的消息结构并不全部一一对应，有一部分本项目针对性新增的。具体存在哪些字段，以及字段的含义都不一定能按照官方的消息结构来理解。
...
1. 扩展的伪字段。这种字段表达的结构可能是错误的但逻辑能成立，例如 `message.text.size`。实际上在真实消息数据中，`text` 已经是一个不可分的字符串内容了，不存在更具体的字段。这里的 `size` 可理解为对 `text` 内容的求总长操作。
