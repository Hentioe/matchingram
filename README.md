...
| `compile_rule` |       `1mb-rule`        | 12.512 ms | 1MB 大小的规则编译（解析）                 |
| `rule_match`   |    `worst-1mb-rule`     | 13.216 ms | 1MB 大小的规则匹配（匹配到末尾的最糟情况） |
...
例如匹配消息文本（`message.text`）或匹配消息来源（`message.from`）。不过消息来源自身没有可匹配的内容，应该访问其具体字段，例如：来源用户是否是机器人（`message.from.is_bot`）或来源用户的全名（`mesage.from.fullname`）。
